...
export default function AreaAnalysis({
  ...
  const [analysisType, setAnalysisType] = useState<"emergency" | "living">(
    "emergency",
  );
  const [inputMethod, setInputMethod] = useState<"pincode" | "location">(
    "pincode",
  );
  ...
  const handleUseLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          setLoading(true);
          const response = await areaRatingApi.analyzeByLocation(
            latitude,
            longitude,
            analysisType,
          );
          ...
          const errorMessage =
            err instanceof Error ? err.message : "Failed to analyze location";
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await areaRatingApi.analyzeByPincode(
        pincode,
        analysisType,
      );
      setResult(response);
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to analyze pincode";
      setError(errorMessage);
    } finally {
      setLoading(false);
    }
      ...
      {result && (
        <div className="space-y-4 pt-4 border-t border-gray-100">
          ...
          <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
            <div
              className={`w-14 h-14 ${getGradeColor(result.grade)} rounded-full flex items-center justify-center text-white text-xl font-bold shadow-lg`}
            >
              {result.grade}
            </div>
            <div className="flex-1">
              ...
              <div className="text-gray-500 text-sm">
                {result.analysis_type === "emergency"
                  ? "Emergency Response"
                  : "Living Condition"}{" "}
                Score
              </div>
              {result.pincode_info && (
                <div className="text-gray-600 text-xs mt-1">
                  📍 {result.pincode_info.place_name},{" "}
                  {result.pincode_info.district}
                </div>
              )}
            </div>
          </div>
