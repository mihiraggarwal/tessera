...
// Route Voronoi Types
export interface RouteVoronoiConfig {
  grid_density?: number;
  base_k?: number;
  adaptive_k?: boolean;
...
export interface RouteVoronoiRequest {
  facilities: Facility[];
  clip_to_india: boolean;
...
export interface RouteVoronoiCompareRequest {
  facilities: Facility[];
  clip_to_india: boolean;
  state_filter?: string | null;
...
export interface RouteVoronoiCompareResponse {
  euclidean_voronoi: GeoJSONFeatureCollection;
  route_voronoi: GeoJSONFeatureCollection;
  comparison: {
    facility_metrics: Array<{
      facility_id: string;
      euclidean_area_km2: number;
      route_area_km2: number;
      area_change_pct: number;
    }>;
    route_metadata: {
      avg_distortion_ratio: number;
      max_distortion_ratio: number;
...
export const voronoiApi = {
  ...
  computeRoute: async (
    request: RouteVoronoiRequest,
  ): Promise<GeoJSONFeatureCollection> => {
    const response = await api.post("/api/voronoi/compute-route", request, {
  ...
  compare: async (
    ...
    const response = await api.post("/api/voronoi/compare", request, {
      timeout: 300000,
    });
    return response.data;
  },

  getRoutingHealth: async (): Promise<RoutingHealthResponse> => {
    const response = await api.get("/api/voronoi/routing-health");
    return response.data;
  },
  ...
  updateRoutingConfig: async (
    ...
    const response = await api.post("/api/voronoi/routing-config", config);
    return response.data;
  },
};
...
export const uploadApi = {
  ...
  getAvailableFiles: async (): Promise<{
    user_data: string[];
  ...
  loadFile: async (filename: string): Promise<UploadResponse> => {
    const response = await api.get(
      `/api/upload/load-file/${encodeURIComponent(filename)}`,
    );
    return response.data;
  },
  ...
  loadPublicFile: async (filename: string): Promise<UploadResponse> => {
    const response = await api.get(
      `/api/upload/load-public-file/${encodeURIComponent(filename)}`,
    );
    return response.data;
  },
  ...
  getBusStops: async (stateName: string): Promise<UploadResponse> => {
    const response = await api.get(
      `/api/upload/bus-stops/${encodeURIComponent(stateName)}`,
    );
    return response.data;
  },
  uploadRawCSV: async (
...
export const boundariesApi = {
  ...
  getStateBoundary: async (stateName: string): Promise<GeoJSONFeature> => {
    const response = await api.get(
      `/api/boundaries/states/${encodeURIComponent(stateName)}`,
    );
    return response.data;
  },
};
...
export const areaRatingApi = {
  analyzeByPincode: async (
    pincode: string,
    analysisType: "emergency" | "living",
  ): Promise<AreaRatingResponse> => {
  ...
  searchPincodes: async (
    query: string,
    limit: number = 10,
  ): Promise<{ results: PincodeInfo[] }> => {
    const response = await api.get(
      `/api/rating/pincode/search/${query}?limit=${limit}`,
    );
    return response.data;
  },

  reverseGeocode: async (lat: number, lng: number): Promise<PincodeInfo> => {
    const response = await api.post("/api/rating/reverse-geocode", {
      lat,
      lng,
    });
    return response.data;
  ...
  getDatasets: async (
    analysisType: "emergency" | "living",
  ): Promise<{ analysis_type: string; datasets: string[] }> => {
